...
**触发词：** 「回填数据」 + 标题关键词（如「回填数据 道家养生」）

**执行流程：**

1. 在**内容生成库**中按标题关键词搜索，找到匹配记录
2. 展示当前已录入的数据（如果有）
3. 请用户提供：发布时间、阅读量、点赞数、收藏数、评论数、分享数
4. 根据 `references/tables.md` 中爆款评级规则自动计算 S/A/B/C/D
5. 更新记录字段：发布时间、发布状态→「已发布」、各项数据、爆款评级
6. 回复确认更新成功
...
**触发词：** 「复盘」、「数据分析」、「复盘上周」、「复盘本月」

**执行流程：**

1. 从**内容生成库**拉取「已发布」且有数据的记录
   - 复盘上周→ filter: 发布时间在上周范围内
   - 复盘本月→ filter: 发布时间在本月范围内
   - 不指定时间→ 取最近 30 条
2. 按以下维度分析：

   **整体表现**
   - 共发布 N 条，S/A/B/C/D 各几条
   - 平均点赞、平均收藏、平均阅读

   **爆款规律**（S+A 级的共性）
   - 选题方向共性：哪个方向进入爆款最多
   - 标题格式共性：疑问句/数字开头/情绪化等
   - 发布时间共性：早上/晚上哪个时段效果好
...
4. 将复盘结论写入 `memory/xhs-review-{YYYY-MM}.md`，方便下次选题时参考
