...
仿写方向已由原文确定，**跳过 Step 1 和 Step 2 的所有人工确认**，直接走完分析并进入生成。
...
3. 提取标题、正文、博主信息、互动数据
4. 写入**选题库**（所有字段，见 `references/tables.md`）
5. **自动执行五维分析**（无需用户确认）：
   - 用原文方向作为唯一方向（不生成方向 1/2 供选择）
   - 自动填充选题分析表（selectedDirection = 「仿写原文方向」）
   - 写入**选题分析表**
6. 写状态：`step=3, selectedTopic=原文标题, selectedDirection=「仿写：{原文标题}」`
7. **直接跳到 Step 3**（生成文案），无停顿
...
4. **选题从选题库取**，不是选题分析库
5. **配图失败不阻断流程**，跳过继续
6. **仿写模式绝对不能在 Step 2 停下来等用户输入**，必须自动完成分析并直接进入 Step 3
7. **仿写模式不生成「方向1/方向2」供用户选择**，只有一个方向（原文方向），自动填入并继续
